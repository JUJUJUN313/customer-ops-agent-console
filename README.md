...
这个项目使用本地 API、服务端 JSON 状态、本地消息入口、触达草稿队列、企微会话内容存档入站骨架、企微智能机器人长连接/测试群机器人连接器、个人微信单账号 AccountAgent + SendScheduler Mock 队列和规则型 Agent，把“电销筛选 → 销售承接 → VIP维护 → 报价订阅 → 数据回流”的本地闭环跑起来。当前系统会真实写入本地客户档案、任务、报价、会话、触达草稿、模型连接配置、企微配置/日志/群绑定、个人微信群上下文/发送队列、Agent运行记录和审计日志；不会真实外呼、发短信或同步CRM。默认Agent先由本地规则引擎稳定执行，点击模型连接测试或在Agent控制台勾选真实LLM增强时，会调用你配置的LLM；配置企微Webhook后，已确认草稿可以真实发送到企微测试群机器人；配置企微会话存档入口后，可以用统一入站接口模拟存档回读消息；配置企微智能机器人 Bot ID/Secret 并启动 bridge 后，可以读取真实企微智能机器人消息并按 `chatid` 独立归档客户群。
...
- 企微会话存档入站会转成统一 `InboundGroupMessage`，进入同一套群上下文、去重、AccountAgent和SendScheduler链路。
- 个人微信单账号 AccountAgent 会按 `roomId` 维护外部群上下文，低风险消息生成自动发送队列，高风险消息进入人工确认，连续客户消息会合并，重复消息会去重，员工回复会取消待发；SendScheduler执行同群FIFO、账号并发、分钟上限、过期重判、失败退避和回读确认。
...
- 企微接入：保存企微会话内容存档入口配置、智能机器人 Bot ID/Secret、启动长连接 bridge 读取真实消息、按 `chatid/roomId` 独立归档客户群、维护群聊绑定；同时保留企微测试群机器人Webhook发送测试消息、已确认草稿灰度推送，以及个人微信单账号 AccountAgent + SendScheduler Mock 队列。Webhook、Bot ID、Secret和入站Secret只在服务端状态保存，前端不回显明文。
...
- `POST /api/wecom/archive/inbound`：写入企微会话内容存档标准化入站消息，按 `roomId/chatId` 去重、归档并进入AccountAgent。
...
- `POST /api/personal-wechat/send-scheduler/run`：运行个人微信 SendScheduler，按账号并发、分钟上限和同群FIFO调度 `queued` 任务为 `sent`。
- `POST /api/personal-wechat/send-jobs/:jobId/confirm`：模拟个人微信发送成功后的自回显/存档确认，写入本地会话、事件和日志。
- `POST /api/personal-wechat/send-jobs/:jobId/fail`：记录个人微信Gateway发送失败、失败退避和人工接管提示。
...
- 企微会话内容存档当前为标准化入站骨架：可以验证存档消息去重、游标回写、群绑定和AccountAgent链路；尚未接真实拉取、解密SDK和客户同意校验。
- 个人微信单账号 AccountAgent 当前为 Mock Gateway + SendScheduler：可以验证多外部群上下文、低风险自动排队、高风险人工确认、连续消息合并、同群FIFO、账号并发、队列过期重判、分钟限流、失败退避和回读确认；尚未接真实个人微信登录、收发、掉线重连和生产合规审批。
- 不接入真实外呼、短信、CRM、交易系统。
