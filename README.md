...
当前版本是 **本地可运行验证版**：系统会真实写入本地客户档案、任务、报价、会话、触达草稿、模型配置、企微配置、个人微信AccountAgent队列、Agent运行记录和审计日志；不会真实外呼、发短信、发企微私聊或同步CRM。外部触达默认都先进入本地任务、草稿、测试群或Mock发送队列；个人微信真实发送必须显式配置出站 Sidecar，并在回读确认前保持“待确认”状态。
...
| VIP群管理 | VIP客户、VIP会话工作台、报价订阅、本地/企微/个人微信会话聚合、VIP分流Agent、@对象识别、售前/售后/报价/投诉任务分流 | 图片、文件、撤回、引用回复和真实已读仍待扩展 |
...
| 企微接入 | 接入总控、会话内容存档标准入站、Sidecar健康检查、智能机器人长连接、测试群Webhook、群绑定搜索、日志筛选、消息去重和脱敏配置 | 官方会话存档SDK或第三方协议服务需在Sidecar内完成拉取/解密/登录态 |
| 个人微信AccountAgent | 单账号多群上下文、低风险队列、高风险人工确认、人工放行、连续消息合并、SendScheduler、Sidecar出站回调、失败退避、回读确认 | 默认Mock；真实登录、协议发送和自回显监听由外部Sidecar承担 |
| 稳定性与审计 | 输入校验、状态自检、敏感信息脱敏、静态访问拦截、审计日志、59个自动化测试 | 本地JSON不是生产数据库 |
...
- `ChatSession`：从会话存档、智能机器人、个人微信和本地模拟会话投影出的统一聊天工作台会话。
...
启动个人微信出站 Sidecar 调度器：

```bash
npm run personal-wechat:gateway
```

只检查个人微信出站 Sidecar 配置：
...
1. 会话内容存档 Gateway：生产主读取入口，对应 `npm run wecom:archive`、`/api/wecom/archive/status` 和 `/api/wecom/archive/inbound`。
2. 智能机器人长连接：用于真实读取企微智能机器人消息，按 `chatid` 独立归档，作为辅助/测试读取入口。
3. 测试群机器人Webhook：只用于发送测试消息和已确认草稿，不能读取群消息。

企微/微信协议服务可作为 Sidecar 对接，建议只暴露标准动作：实例健康、登录态、消息回调、发送文本/群@、标记已读、联系人/群同步、文件CDN下载。系统内部不直接耦合某个协议供应商，所有入站先转成统一 `roomId/messageId/senderType/text/source` 结构。

### 业务会话工作台

- 聚合企微会话存档、企微智能机器人、个人微信AccountAgent和本地模拟消息。
- 电销管理下的“电销会话”只看电销企微培育、个人微信托管号和本地电销私聊。
- 销售管理下的“销售会话”只看会员卡销售承接相关的企微、个人微信和本地销售私聊。
...
当前个人微信链路支持 Mock 验证和 Sidecar 出站骨架：
...
- Mock模式下调度后进入 `sent_pending_confirm`，只用于本地流程验证。
- Sidecar模式下调度后进入 `sending`，由 `npm run personal-wechat:gateway` 调用外部发送服务；Gateway回调后进入 `sent_pending_confirm`。
...
| 业务会话工作台 | 已可运行 | 电销、销售、VIP各有入口和默认会话范围，底层统一聚合企微/个人微信/本地消息，支持绑定客户和回复入队 |
| 企微会话内容存档 | Gateway骨架完成 | 真实SDK/协议拉取、解密和客户同意校验由Sidecar适配 |
| 个人微信AccountAgent | Mock闭环 + Sidecar出站骨架完成 | 真实登录、协议收发、自回显监听和生产风控待接 |
...
- 个人微信AccountAgent、Gateway健康检查、人工放行、SendScheduler、Sidecar出站回调、失败退避和回读确认。
