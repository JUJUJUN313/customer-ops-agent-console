...
- 企微后台群发任务支持从电销分层创建批次、同批次幂等复用、人工审批入队、专用Chrome CDP执行器就绪后调度、提交前验证结果回写、客户群入口和正式提交门禁。
- `submitMode=submit` 任务在执行器未声明 `supportsSubmit=true` 时保持队列，不会派发到本地执行器。
- 企微后台群发任务会拒绝缺少目标客户或文案的请求；部门筛选默认使用“电销部门”，指定员工是可选收窄条件。
...
65. 企微后台群发任务：创建、审批、调度、提交前验证回写和客户群入口均不应影响企微客户端实时收发队列；CDP 后台控制不抢企微前台
66. 异常输入：非法客户阶段、客户风险、任务状态、任务优先级、负责人角色、消息发送角色、SLA参考时间、空全局模型、非法API URL、非法销售结果、重复报价、非法销售样本、空渠道消息、非法触达草稿渠道、非法触达草稿状态、伪造企微已发送、未确认草稿发送企微、空草稿内容、空批量任务ID、空批量草稿ID、重复企微会话存档消息ID、重复企微客户端实时消息ID、重复个人微信消息ID、未知个人微信群ID、未知会话ID、无效会话绑定客户、高风险会话回复、未放行高风险任务直接确认、`queued`任务直接确认、Sidecar缺配置调度、企微后台群发缺客户/缺文案
...
- 电销企微后台群发任务创建后默认为待审批，审批后进入待派发；`submitMode=dry-run` 时专用Chrome CDP执行器只做提交前验证并回写 `dry_run_passed`，正式提交成功时回写 `submitted`。
- 群发执行器未声明 `canDispatch=true` 时，调度不会把任务改成派发中；正式提交任务在 `supportsSubmit=false` 时保持队列；两者都不会影响企微客户端实时收发 Worker。
