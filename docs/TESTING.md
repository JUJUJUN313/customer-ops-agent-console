...
- 个人微信 Sidecar 出站模式会先进入 `sending`，Gateway回调后进入 `sent_pending_confirm`。
...
36. `GET /api/outbound-drafts`
37. `POST /api/outbound-drafts`
38. `POST /api/outbound-drafts/:draftId/status`
39. `POST /api/outbound-drafts/:draftId/send-wecom`
40. `POST /api/outbound-drafts/batch-status`
41. `POST /api/tasks/escalate`
42. `POST /api/tasks/:taskId/status`
43. `POST /api/tasks/batch-status`
44. `POST /api/sales-samples`
45. `npm run wecom:archive -- --check`
46. `npm run personal-wechat:gateway -- --check`
47. `npm run wecom:bridge -- --check --timeout=20000`
48. 异常输入：非法客户阶段、客户风险、任务状态、任务优先级、负责人角色、消息发送角色、SLA参考时间、空全局模型、非法API URL、非法销售结果、重复报价、非法销售样本、空渠道消息、非法触达草稿渠道、非法触达草稿状态、伪造企微已发送、未确认草稿发送企微、空草稿内容、空批量任务ID、空批量草稿ID、重复企微会话存档消息ID、重复个人微信消息ID、未知个人微信群ID、未知会话ID、无效会话绑定客户、高风险会话回复、未放行高风险任务直接确认、`queued`任务直接确认、Sidecar缺配置调度
...
- `/api/personal-wechat/gateway/check` 在Mock模式下应记录检查通过；Sidecar模式缺URL或不可达时应写入错误，不触发外部发送。
- 会话工作台列表应能展示企微会话存档、企微智能机器人、个人微信和本地模拟会话；不同 `roomId/chatId/sessionId` 不得混到同一客户，电销/销售/VIP入口不得互相显示不相关会话。
...
- Sidecar模式下，运行 SendScheduler 后任务应先变为 `sending`；出站Gateway调用 `dispatched` 后才变为 `sent_pending_confirm`。
...
- 逐个点击一级业务分组和二级菜单，确认 `#viewTitle` 与页面内容正常；总览、客户管理、电销管理、销售管理、VIP群管理和设置均可展开；企微接入和消息入站应在设置下；电销会话、销售会话、VIP会话标题和业务范围提示应分别正确。
...
- 在企微接入页确认顶部“接入总控”展示会话存档、智能机器人、测试群发送和个人微信出站四张状态卡。
- 在企微接入页点击“检查存档Sidecar”和“检查个微Gateway”，确认结果回写到状态卡和日志；缺配置时显示明确缺项。
- 在企微接入页发送已确认草稿后，确认草稿状态变为 `企微已发送`，最近企微记录出现成功日志。
- 在企微接入页写入模拟企微入站，确认会话和Agent输出刷新。
- 在企微接入页写入企微会话内容存档模拟入站，确认会话存档状态、群上下文和发送队列刷新。
...
- 在会话工作台切换到本地模拟会话回复，确认只生成本地触达草稿，不显示真实外发状态。
- 在企微接入页个人微信区域写入低风险外部群消息，确认出现待自动发送任务；点击“运行发送调度”后Mock模式任务变为已提交待回读；点击“回读确认”后，确认任务变为已确认。
- 在企微接入页个人微信区域切换Sidecar模式并保存后，写入低风险消息，运行调度后任务应变为发送中；外部Gateway回调后变为已提交待回读。
...
- 浏览器自动化验收：业务页面完成导航遍历，均可渲染、无框架错误、无重复DOM id、无控制台报错；企微接入页可进入“设置 > 企微接入”；接入总控四张状态卡、两个健康检查按钮、群绑定搜索框和日志筛选控件均唯一渲染；“检查个微Gateway”返回 Mock 检查通过，“检查存档Sidecar”在缺少 CorpID/Secret/私钥时给出明确缺项；“查看发送测试”跳转到发送测试卡片并更新 `#wecom-send-test` hash。当前浏览器插件的文本输入和截图能力受虚拟剪贴板/CDP超时限制，筛选输入和截图需后续手工补验。
