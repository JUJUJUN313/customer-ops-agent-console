...
本项目是“智能客服与客户运营中台”的本地可运行版。当前阶段使用本地 API、JSON 状态存储、触达草稿队列、企微后台群发派发队列、企微会话内容存档Sidecar读取契约、企微客户端实时未读Worker契约、AccountAgent + SendScheduler 和规则型 Agent，把从电销分层、销售承接、VIP群维护、报价订阅到数据回流的业务链路跑通。系统会真实写入本地状态；不会真实外呼、发短信或同步CRM。当前业务界面按三线能力重组：电销运营负责历史消息分析、客户分层、群发计划和企微后台群发任务派发；销售运营负责优先级任务、客户摘要和推荐回复；VIP群维护负责实时企微收发、高风险确认和历史补账。默认Agent不调用外部大模型，只有模型连接测试或勾选真实LLM增强时才会调用已配置模型；配置企微客户端实时Worker后，可以通过 `/messages` 读取本地脚本识别到的未读外部群，由系统维护静默窗口和会话版本，再通过 `/send` 执行企微员工客户端自动化发送，最终由服务商历史下载补账确认。
...
- 电销运营：展示电销客户池、客户分层、历史消息分析结果、推荐群发批次和审批入口；可创建企微后台客户/客户群群发任务、审批入队并派发给专用Chrome CDP执行器，系统确认后默认正式提交，不抢企微客户端前台；群发任务以部门筛选为主，员工可选收窄，客户群任务会做群全名关键词和相似群排除保护。
...
- 扩充测试到 81 个用例，覆盖正常链路、异常输入、脏数据、销售学习、消息上下文、闭环编排、任务SLA、任务批量处理、触达草稿批量处理、企微后台群发任务创建/审批/调度/提交前验证回写/客户群入口/正式提交门禁、状态集合缺失恢复、LLM/语音模型配置、API Key保留/清空、真实LLM连接测试、Agent执行边界、企微配置脱敏、企微测试发送、企微草稿发送、企微入站兼容接口、会话存档标准入站、会话存档非文本占位任务、消息投影/绑定/回复、会话存档Gateway状态回写、智能机器人凭据脱敏、按 `chatid/roomId` 独立建档、`msgid/messageId` 去重、机器人出站回写幂等，以及企微客户端实时Worker、`canReceive/canSend/supportsAck`能力、确认回执ACK、重复回读确认幂等、低风险队列、高风险人工确认/人工放行、连续消息合并、SendScheduler调度、发送回调、失败退避、队列过期取消和分钟限频。
