...
当前系统不会自动执行真实外呼、短信、CRM同步、企微私聊发送或个人微信真实发送。只有在配置企微测试群机器人Webhook后，人工确认的草稿或测试消息才会真实发送到企微测试群机器人。企微会话内容存档当前提供标准化入站骨架，可验证外部VIP群消息读取、去重、群归档和AccountAgent链路；企微智能机器人长连接启动后，可以读取真实企微智能机器人消息并按 `chatid` 独立归档，定位为辅助/测试读取入口。
...
1. 企微会话内容存档标准入站：生产主读取入口的本地可验证骨架，用于接收WeComArchiveGateway拉取解密后的标准消息。
...
2. 设置默认客户、默认渠道和游标。
3. 保存企微配置。
4. 在“模拟企微会话存档入站”区域填写群ID、群名、发送人、客户消息和消息ID。
5. 点击“写入存档入站”。
6. 查看会话存档状态、个人微信群上下文、AccountAgent决策和发送队列是否刷新。

会话存档接口只接收标准化后的消息，不负责真实企微拉取、解密或客户同意校验。生产接入时应由独立的 `WeComArchiveGateway` 完成拉取、解密、合规检查和游标管理，再调用 `/api/wecom/archive/inbound`。
...
4. 点击“运行发送调度”，确认任务变为“已发送待确认”。
5. 点击“模拟回读确认”，确认任务变为“已确认”，群会话和客户事件出现AccountAgent回复。
6. 写入报价、锁价、退款、赔偿、付款或合同类消息，确认进入人工确认。
7. 写入员工或托管号回复，确认同群待发任务被取消。

当前边界：

- 不连接真实个人微信。
- 不真实发送个人微信消息。
- SendScheduler只是Mock状态机和流程验证，真实Gateway接入前不会发出个人微信消息。
...
| 企微会话内容存档标准入站 | 本地骨架可用 | 否，只读取和归档标准化消息 |
| 企微智能机器人长连接读消息 | 已可用，需配置Bot ID/Secret并运行bridge | 只读取，不自动回复 |
| 个人微信AccountAgent + SendScheduler | Mock可运行 | 否 |
| 外呼、短信、CRM同步 | 未接入 | 否 |
...
当前系统已经能验证本地业务闭环、模型增强、企微测试群发送、企微会话存档标准入站、企微智能机器人长连接读取、`chatid/roomId` 独立归档和个人微信AccountAgent + SendScheduler Mock流程。
