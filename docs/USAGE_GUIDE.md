...
首次启动后，在这个专用 Chrome Profile 里登录企微后台。该窗口由 CDP 后台控制，正常派发时不会抢企微客户端前台、鼠标、键盘或 AX。
...
群发执行器默认按系统确认后的 `submitMode=submit` 任务正式提交企微后台群发；显式设置 `submitMode=dry-run` 时只做提交前验证，不点击最终按钮。需要临时停用真实提交时，用 `WECOM_ADMIN_ALLOW_SUBMIT=false` 启动执行器。客户任务进入“群发消息给客户”，客户群任务进入“群发消息到企业的客户群”；主筛选是部门，员工只作为可选收窄条件。客户群会按群全名关键词筛选，并自动把已知相似群放入“不发送给”排除词；客户入口如果无法确认只命中指定客户，会拦截提交。首次使用需要在专用 Chrome 里人工登录企微后台，登录态正常后系统会自动派发。
...
- `电销运营 > 分析与群发`：查看电销客户池、客户分层、历史消息分析结果、推荐群发批次；群发计划先创建企微后台群发任务，再经业务确认后派发给后台浏览器执行器。
...
2. 在批次卡片里确认部门筛选；系统默认填“电销部门”，可人工修改，指定员工只是可选收窄条件。
3. 点击“创建企微群发任务”，任务会进入“企微后台群发派发”区域，状态为待审批。
4. 点击“确认并入队”，任务进入待派发队列。
5. 启动本地企微后台执行器和派发 Worker 后，点击“检查企微后台”和“派发下一条”，或让 Worker 常驻自动派发。
6. Worker 会通过专用 Chrome CDP 进入企微后台，按任务对象类型自动选择客户入口或客户群入口并提交。客户群任务会同时配置部门、可选员工、群全名关键词和自动排除词；客户和客户群任务都会在正式提交前做范围保护；如需只验证不提交，可把任务设为 `submitMode=dry-run`。
...
| 企微后台群发任务派发 | 已可用，使用专用Chrome Profile + CDP后台控制；系统确认后默认正式提交，支持提交前验证、部门筛选和客户群相似名称排除保护 | 正式提交会在企微后台创建群发任务；提交前验证不会 |
...
当前系统已经能验证本地业务闭环、模型增强、电销企微后台群发任务提交前验证/正式派发、企微测试群发送、企微会话存档读取契约、`/ack`游标确认、企微智能机器人长连接读取、`chatid/roomId` 独立归档、企微客户端Worker接收/ACK/发送能力声明、AccountAgent兼容队列 + SendScheduler流程，以及本地动作回执和回读确认骨架。
