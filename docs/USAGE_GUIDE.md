...
群发执行器默认按系统确认后的 `submitMode=submit` 任务正式提交企微后台群发；显式设置 `submitMode=dry-run` 时只做提交前验证，不点击最终按钮。需要临时停用真实提交时，用 `WECOM_ADMIN_ALLOW_SUBMIT=false` 启动执行器。客户任务进入“群发消息给客户”，客户群任务进入“群发消息到企业的客户群”；客户群入口按群名关键词筛选，关键词不是唯一命中时会拦截提交。
...
6. Worker 会通过专用 Chrome CDP 进入企微后台，按任务对象类型自动选择客户入口或客户群入口并提交。客户群任务使用群名关键词筛选，系统会在正式提交前做唯一性保护；如需只验证不提交，可把任务设为 `submitMode=dry-run`。
