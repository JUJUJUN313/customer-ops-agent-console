...
群发执行器默认按系统确认后的 `submitMode=submit` 任务正式提交企微后台群发；显式设置 `submitMode=dry-run` 时只做提交前验证，不点击最终按钮。需要临时停用真实提交时，用 `WECOM_ADMIN_ALLOW_SUBMIT=false` 启动执行器。客户任务进入“群发消息给客户”，但企微后台只能按员工添加客户/标签等范围筛选，无法确认只命中指定客户时会拦截提交；客户群任务进入“群发消息到企业的客户群”，会选择任务里的员工/群主并按群名关键词筛选，关键词不是唯一命中时也会拦截提交。首次使用需要在专用 Chrome 里人工登录企微后台，登录态正常后系统会自动派发。
...
6. Worker 会通过专用 Chrome CDP 进入企微后台，按任务对象类型自动选择客户入口或客户群入口并提交。客户群任务会同时配置群主/员工和群名关键词；客户和客户群任务都会在正式提交前做范围保护；如需只验证不提交，可把任务设为 `submitMode=dry-run`。
...
| 企微后台群发任务派发 | 已可用，使用专用Chrome Profile + CDP后台控制；系统确认后默认正式提交，支持提交前验证和客户群关键词唯一性保护 | 正式提交会在企微后台创建群发任务；提交前验证不会 |
