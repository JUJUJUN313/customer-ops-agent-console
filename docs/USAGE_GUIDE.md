...
当前系统不会自动执行真实外呼、短信、CRM同步或企微私聊发送。只有在配置企微测试群机器人Webhook后，人工确认的草稿或测试消息才会真实发送到企微测试群机器人；个人微信真实发送必须显式配置出站Sidecar，且只有回读确认后才算闭环完成。企微会话内容存档当前提供 Gateway 和标准化入站骨架，可验证外部VIP群消息读取、去重、群归档、会话工作台和AccountAgent链路；真实拉取、解密、登录态、客户同意校验、图片/文件下载和协议适配应由独立Sidecar完成。企微智能机器人长连接启动后，可以读取真实企微智能机器人消息并按 `chatid` 独立归档，定位为辅助/测试读取入口。
...
- “电销管理 > 电销会话”：只看电销沉淀企微、个人微信托管号和本地电销私聊中归属电销流程的会话。
- “销售管理 > 销售会话”：只看销售企微、个人微信托管号和本地销售私聊中归属会员卡销售承接的会话。
...
1. 企微会话内容存档 Gateway：生产主读取入口的本地可验证骨架，用于通过Sidecar拉取/解密消息，再写入标准入站接口。
...
- “个人微信出站”会显示 Mock/Sidecar 模式、待发/发送中/待回读/人工确认队列，并可点击“检查个微Gateway”。
...
3. 保存企微配置。
4. 本地调试时可以先在“模拟企微会话存档入站”区域填写群ID、群名、发送人、客户消息和消息ID。
5. 点击“写入存档入站”，确认会话工作台、群上下文、AccountAgent决策和发送队列刷新。
6. 接真实Sidecar时，在终端运行 `npm run wecom:archive -- --check` 检查配置。
7. 检查通过后运行 `npm run wecom:archive` 持续轮询Sidecar `/pull`。
8. Gateway会把连接、拉取、解密、错误和游标状态回写到 `/api/wecom/archive/status`。
9. 在页面“接入总控”点击“检查存档Sidecar”，确认Sidecar健康状态能回写到页面。

如果 `npm run wecom:archive -- --check` 发现配置缺失，终端会输出 JSON，例如 `ok=false`、`missing=["corpId","archiveSecret","privateKey"]`，并返回非0退出码；这表示配置尚未满足真实拉取条件，不会伪装成已连接。
...
### 6.12 个人微信 AccountAgent

企微接入页还有个人微信单账号 AccountAgent 区域，用于验证“一个托管号加入多个外部群，一个账号对应一个Agent”的产品链路。当前支持 Mock 本地验证和 Sidecar 出站骨架；真实个人微信登录、协议发送、二维码托管、自回显监听和账号风控由外部Sidecar承担。
...
- Sidecar模式下调度后进入“发送中”，由 `npm run personal-wechat:gateway` 调用外部Sidecar；Sidecar成功回调后进入“已提交待回读”。
- 回读确认后才写入本地会话和客户事件。
...
3. 运行 `npm run personal-wechat:gateway -- --check`，确认配置可读取。
...
7. 在页面“接入总控”点击“检查个微Gateway”，确认 Mock 或 Sidecar 健康状态能写入个人微信运行日志。

当前边界：

- 默认Mock模式不连接真实个人微信，也不真实发送个人微信消息。
- Sidecar模式只负责把 `sending` 任务交给外部发送服务，外部Sidecar必须自己保证登录态、发送成功、自回显监听、风控和失败回调。
...
| 业务会话工作台 | 已可用 | 否，低风险只入队，高风险人工确认，真实发送需出站Gateway |
| 企微会话内容存档Gateway | 骨架可用，需Sidecar | 否，只读取、归档和回写游标 |
| 企微智能机器人长连接读消息 | 已可用，需配置Bot ID/Secret并运行bridge | 只读取，不自动回复 |
| 个人微信AccountAgent + SendScheduler | Mock可运行，Sidecar出站骨架可接 | 默认否；Sidecar配置后可调用外部发送服务，但回读确认前不算完成 |
| 外呼、短信、CRM同步 | 未接入 | 否 |
...
- Gateway模式为 `disabled` 或 Sidecar地址为空，调度不会提交发送。
...
当前系统已经能验证本地业务闭环、模型增强、企微测试群发送、企微会话存档标准入站、企微智能机器人长连接读取、`chatid/roomId` 独立归档、个人微信AccountAgent + SendScheduler Mock流程，以及个人微信Sidecar出站回调骨架。
