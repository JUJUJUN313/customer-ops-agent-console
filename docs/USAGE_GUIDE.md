...
- `电销运营 > 分析与群发`：查看电销客户池、客户分层、历史消息分析结果、推荐群发批次；群发计划先进入人工确认，再接企微后台群发任务创建。
...
当前系统已经能验证本地业务闭环、模型增强、企微测试群发送、企微会话存档读取契约、`/ack`游标确认、企微智能机器人长连接读取、`chatid/roomId` 独立归档、企微客户端Worker接收/ACK/发送能力声明、AccountAgent兼容队列 + SendScheduler流程，以及本地动作回执和回读确认骨架。
