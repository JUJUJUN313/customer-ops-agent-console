...
但它还不是生产级智能客服系统。真实外呼、短信、交易系统、CRM、人员排班、客户联系加好友回调、企业通讯录和生产级权限策略仍未接入。企微已经支持会话内容存档Gateway骨架、标准化入站、本地入站适配、测试群机器人Webhook发送和智能机器人 WebSocket 长连接辅助读取；会话工作台已经按电销、销售、VIP三个业务入口拆分展示，并把企微、个人微信和本地消息统一成业务可操作的聊天界面；个人微信单账号 AccountAgent 已完成 Mock 入站、风控决策、人工放行、同群上下文合并、SendScheduler受控调度、Sidecar发送回调、失败退避和回读确认闭环；LLM已支持OpenAI兼容连接测试和Agent按需增强，但还没有生产级内容安全、成本统计、知识库版本控制和权限审批。当前更准确的定位是：本地工作台真实可用，会话存档主读取链路已有可验证骨架，企微智能机器人长连接已认证可用，个人微信链路可做内部闭环和出站Sidecar联调验证，生产级外部触达和系统集成仍需继续建设。
...
| 企微连接器 | 标准入站/长连接可运行 | 可保存企微会话内容存档入口配置，接收 `/api/wecom/archive/inbound` 标准化消息并回写游标；可检查会话存档Sidecar `/health`；可保存企微智能机器人 Bot ID/Secret 并脱敏展示，bridge 使用 WebSocket 长连接辅助读取消息，按 `chatid` 独立归档客户群，按 `msgid` 去重；同时支持测试群机器人Webhook发送测试消息和已确认草稿，群绑定可搜索，日志可按类型筛选 | 会话存档真实拉取、解密SDK、客户同意校验、客户联系加好友回调、企业通讯录、成员私聊全量同步、生产权限策略和发送审批仍未完成 |
| 业务会话工作台 | 本地可用 | 电销、销售、VIP各有入口和默认业务范围；聚合企微会话存档、企微智能机器人、个人微信AccountAgent和本地模拟会话；支持搜索、来源/状态筛选、三栏聊天、客户绑定、任务/报价/队列查看，外部群回复按风险进入SendScheduler或人工确认，本地会话只保存草稿 | 真实出站Gateway、已读/撤回/引用、图片/文件、群成员权限和多人协作审批仍待完善 |
| 个人微信 AccountAgent | Mock可运行/Sidecar骨架可联调 | 单个人微信账号对应一个 AccountAgent 和 SendScheduler；可按 `roomId` 维护多个外部群上下文，低风险自动排队，高风险人工确认与人工放行，连续客户消息合并，重复消息去重，员工/托管号回复取消待发；发送层执行同群FIFO、账号并发、分钟上限、队列过期重判、Gateway健康检查、Sidecar发送回调、失败退避和回读确认 | 当前未接真实个人微信登录、协议收发、掉线重连、自回显监听、生产限频退避和合规审批 |
...
| 外部触达执行器 | 半闭环/未接入 | 配置Webhook后，已确认草稿可发送到企微测试群机器人；个人微信SendScheduler可在Mock模式验证状态机，也可在Sidecar模式把 `sending` 任务交给外部发送服务；其他动作只生成本地任务和文案 | 未接真实外呼、短信、企微私聊、完整个人微信真实Gateway、官方客户群回调、CRM写入 |
...
- 企微/个微接入体验增强：接入页新增接入总控、会话存档Sidecar健康检查、个人微信Gateway健康检查、群绑定搜索和企微日志类型筛选。
- 会话工作台增强：新增 `ChatSession` 投影、聊天会话API和类微信/企微三栏界面，把复杂连接面板里的消息处理入口收敛到业务人员可用的聊天工作台；前端按电销、销售、VIP入口隔离会话范围；会话卡只突出待绑定、待回复、高风险等行动状态；低风险回复进入发送队列，高风险回复进入人工确认，无真实出站通道时只保存草稿或队列状态。
- 会话存档Gateway增强：新增 `scripts/wecom-archive-gateway.mjs`、`npm run wecom:archive` 和 `/api/wecom/archive/status`，由Sidecar处理官方SDK/协议服务、登录态、解密、CDN文件和原始消息适配，主系统只接收标准化消息。
- 个人微信单账号 AccountAgent 增强：新增 `PersonalWechat` 状态、Mock 入站接口、SendScheduler调度接口、人工放行接口、Sidecar发送回调接口、发送失败接口、回读确认接口、群上下文、回复决策、发送队列和日志；验证“一个账号一个Agent、同群FIFO、账号受控并发、连续客户消息合并、超时重判、分钟限频、失败退避、回读确认”的核心约束。

## 仍然不能宣称已实现的能力

- 真实智能外呼：没有语音通话、录音、ASR、打断、多轮外呼对话。
- 真实短信/企微引导：没有短信供应商、企微活码、好友添加回调。
- 真实企微群助手：会话内容存档已有Gateway和标准化入站骨架，智能机器人长连接已经可以作为辅助/测试入口读取真实机器人会话，并按 `chatid` 独立归档；但会话存档Sidecar真实拉取解密、客户同意校验、文件/图片下载入库、撤回、引用回复、群成员权限、客户联系回调和生产级审批仍未完成。
- 真实个人微信托管号：当前已有 Mock Gateway 和 Sidecar出站回调骨架，可验证系统内部 AccountAgent、队列和外部发送服务回调；还没有接入个人微信真实登录、收消息、协议发消息、自回显监听、掉线重连和生产风控。
