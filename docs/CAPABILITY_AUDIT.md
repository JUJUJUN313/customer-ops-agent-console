...
但它还不是生产级智能客服系统。真实外呼、短信、交易系统、CRM、人员排班、客户联系加好友回调、企业通讯录和生产级权限策略仍未接入。企微已经支持会话内容存档标准化入站、本地入站适配、测试群机器人Webhook发送和智能机器人 WebSocket 长连接辅助读取；个人微信单账号 AccountAgent 已完成 Mock 入站、风控决策、同群上下文合并、SendScheduler受控调度、失败退避和回读确认闭环；LLM已支持OpenAI兼容连接测试和Agent按需增强，但还没有生产级内容安全、成本统计、知识库版本控制和权限审批。当前更准确的定位是：本地工作台真实可用，会话存档主读取链路已有可验证骨架，企微智能机器人长连接已认证可用，个人微信链路可做内部闭环验证，生产级外部触达和系统集成仍需继续建设。
...
| 企微连接器 | 标准入站/长连接可运行 | 可保存企微会话内容存档入口配置，接收 `/api/wecom/archive/inbound` 标准化消息并回写游标；可保存企微智能机器人 Bot ID/Secret 并脱敏展示，bridge 使用 WebSocket 长连接辅助读取消息，按 `chatid` 独立归档客户群，按 `msgid` 去重；同时支持测试群机器人Webhook发送测试消息和已确认草稿 | 会话存档真实拉取、解密SDK、客户同意校验、客户联系加好友回调、企业通讯录、成员私聊全量同步、生产权限策略和发送审批仍未完成 |
| 个人微信 AccountAgent | Mock可运行 | 单个人微信账号对应一个 AccountAgent 和 SendScheduler；可按 `roomId` 维护多个外部群上下文，低风险自动排队，高风险人工确认，连续客户消息合并，重复消息去重，员工/托管号回复取消待发；发送层执行同群FIFO、账号并发、分钟上限、队列过期重判、失败退避和回读确认 | 当前未接真实个人微信登录、收发、掉线重连、自回显监听、生产限频退避和合规审批 |
...
| 外部触达执行器 | 半闭环/未接入 | 配置Webhook后，已确认草稿可发送到企微测试群机器人；个人微信SendScheduler当前可验证受控发送状态机但不触发真实外发；其他动作只生成本地任务和文案 | 未接真实外呼、短信、企微私聊、个人微信真实Gateway、官方客户群回调、CRM写入 |
...
- 个人微信单账号 AccountAgent 增强：新增 `PersonalWechat` 状态、Mock 入站接口、SendScheduler调度接口、发送失败接口、回读确认接口、群上下文、回复决策、发送队列和日志；验证“一个账号一个Agent、同群FIFO、账号受控并发、连续客户消息合并、超时重判、分钟限频、失败退避”的核心约束。

## 仍然不能宣称已实现的能力

- 真实智能外呼：没有语音通话、录音、ASR、打断、多轮外呼对话。
- 真实短信/企微引导：没有短信供应商、企微活码、好友添加回调。
- 真实企微群助手：会话内容存档已有标准化入站骨架，智能机器人长连接已经可以作为辅助/测试入口读取真实机器人会话，并按 `chatid` 独立归档；但会话存档真实拉取解密、客户同意校验、文件/图片下载入库、撤回、引用回复、群成员权限、客户联系回调和生产级审批仍未完成。
...
3. 个人微信下一步接真实Gateway时从 `concurrency=1` 小范围灰度，观察乱序、失败率、确认延迟和账号风控后再提升到2-3。
