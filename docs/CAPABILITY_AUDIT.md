...
| 业务会话工作台 | 本地可用 | 电销、销售、VIP各有入口和默认业务范围；默认聚合企微会话存档、企微智能机器人和个人微信/Sidecar真实来源，本地模拟只在“本地调试”筛选中出现；支持搜索、来源/状态筛选、三栏聊天、客户绑定、任务/报价/队列查看，外部群回复按风险进入SendScheduler或人工确认，本地会话只保存草稿 | 已读/撤回/引用、图片/文件原件预览、群成员权限和多人协作审批仍待完善；真实发送依赖统一出站Sidecar |
| 个人微信 AccountAgent / 统一出站Sidecar | Mock可运行/Sidecar可联调 | 单个人微信账号对应一个 AccountAgent 和 SendScheduler；可按 `roomId` 维护多个外部群上下文，低风险自动排队，高风险人工确认与人工放行，连续客户消息合并，重复消息去重，员工/托管号回复取消待发；发送层执行同群FIFO、账号并发、分钟上限、队列过期重判；Sidecar `/health` 必须声明 `canSend=true` 才会真实调度；支持Sidecar发送回调、失败退避和回读确认 | 当前未接真实个人微信登录、协议收发、掉线重连、自回显监听、生产限频退避和合规审批 |
...
- 企微/个微接入体验增强：接入页收敛为真实连接中心，突出会话存档Sidecar、智能机器人测试入口和统一出站Sidecar；模拟入站移入高级调试；健康检查会展示拉取、解密、发送、回读和撤回能力。
...
- 个人微信单账号 AccountAgent 增强：新增 `PersonalWechat` 状态、Mock 入站接口、SendScheduler调度接口、人工放行接口、Sidecar发送回调接口、发送失败接口、回读确认接口、群上下文、回复决策、发送队列和日志；验证“一个账号一个Agent、同群FIFO、账号受控并发、连续客户消息合并、超时重判、分钟限频、失败退避、回读确认”的核心约束，并通过 `canSend=true` 防止无真实发送能力时伪装外发。
...
- 真实个人微信托管号：当前已有 Mock Gateway 和 Sidecar出站回调骨架，可验证系统内部 AccountAgent、队列、`canSend`能力门禁和外部发送服务回调；还没有接入个人微信真实登录、收消息、协议发消息、自回显监听、掉线重连和生产风控。
