...
但它还不是生产级智能客服系统。真实外呼、短信、交易系统、CRM、人员排班、客户联系加好友回调、企业通讯录和生产级权限策略仍未接入。企微已经支持真实连接中心、会话内容存档Sidecar契约、标准化入站、ACK游标确认、测试群机器人Webhook发送和智能机器人 WebSocket 长连接辅助读取；模拟入站已收敛到高级调试，不再作为业务主入口。会话工作台已经按电销、销售、VIP三个业务入口拆分展示，并默认聚合真实来源；个人微信单账号 AccountAgent 已完成 Mock 入站、风控决策、人工放行、同群上下文合并、SendScheduler受控调度、Sidecar能力声明、Sidecar发送回调、失败退避和回读确认闭环；LLM已支持OpenAI兼容连接测试和Agent按需增强，但还没有生产级内容安全、成本统计、知识库版本控制和权限审批。当前更准确的定位是：本地工作台真实可用，企微智能机器人长连接已认证可用，会话存档和统一出站已固定主系统契约，真实SDK/协议拉取、解密、登录态和主动发送仍需外部Sidecar生产化。
...
- 导航信息架构增强：左侧一级菜单按总览、客户管理、电销管理、销售管理、VIP群管理和设置收拢；客户管理保留全量客户信息，电销/销售/VIP各有自己的客户档案入口，报价订阅归VIP群管理，企微接入和消息入站归设置。
- 企微P1/P2增强：新增企微接入页、`WeComConfig`、`WeComBinding`、`WeComLog`、测试发送、已确认草稿发送、企微模拟入站、会话内容存档标准入站、智能机器人长连接 bridge、Webhook/Bot ID/Secret 脱敏、`chatid/roomId` 独立归档、`msgid/messageId` 去重和自检校验。
- 企微/个微接入体验增强：接入页收敛为真实连接中心，突出会话存档Sidecar、智能机器人测试入口和个人微信Sidecar；模拟入站移入高级调试；健康检查会展示拉取、解密、接收、发送、ACK、回读和撤回能力。
