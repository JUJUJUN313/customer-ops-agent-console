...
但它还不是生产级智能客服系统。真实外呼、短信、交易系统、CRM、人员排班、客户联系加好友回调、企业通讯录和生产级权限策略仍未接入。企微已经支持测试群机器人Webhook发送、本地入站适配和智能机器人 WebSocket 长连接读取；LLM已支持OpenAI兼容连接测试和Agent按需增强，但还没有生产级内容安全、成本统计、知识库版本控制和权限审批。当前更准确的定位是：本地工作台真实可用，企微智能机器人长连接已认证可用，生产级外部触达和系统集成仍需继续建设。
